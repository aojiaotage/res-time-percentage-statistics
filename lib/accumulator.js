...
class ResTimeAccumulator {
  constructor({ timeout, ratios }) {
    ...
    // check if ratios array match asc order, and if elements are in range
    ratios.reduce((p, n) => {
  ...
  add(mills) {
    ...
    this.totalCount += 1;
    // no need for initial the element since done in constructor, and ignore the invalid input
  ...
  cal() {
    ...
    for (let i = 0; i < this.dataArray.length; i += 1) {
      // makes if really fast on response-time-like data
      // since only iterate a small number of the indexes
